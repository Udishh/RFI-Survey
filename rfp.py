...
for i in range(256*1024):
    ...
    x.append(a)
power,freq=psd(x, NFFT=1024, Fs=2.5e6/1e6, Fc=100e6/1e6, return_line=False ) 
clf()

#for cutting the stuff

f=[]
p=[]
for i in range(len(freq)):
    if freq[i] > 99 and freq[i] < 100:
        f.append(freq[i])
        p.append(10*math.log10(power[i]))
...
P=np.array(p)
plot(freq,P,color="black")
show()
#for cutting

power[len(power)//2]= ( power[len(power)//2 +1] + power[len(power)//2 -1] )/2
print(power[len(power)//2],power[(1024//2) -1], power[(1024//2) +1])
print(max(power),"jajaja")
plot(freq,power)
show()'''
